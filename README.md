...
(`PASSWORD_JAM`) y la del reto de dibujo (`PASSWORD_INK`). Los docentes son los
mismos —la lista de `config.js`— pero entrar a una no abre las otras. Si a una
le falta su clave en el `.env`, ese panel queda cerrado y todo lo demás
funciona igual.
...
sus inscritos, sus dibujos y su podio—. Para el siguiente no se borra nada, se
abre otra desde `/ink/panel` y la anterior queda archivada tal como quedó.
