...
function activo() {
  ...
  // Un desvío mal escrito cuenta como no poder mandar, y no como poder mandar
  // a la calle: así los botones que solo sirven para mandar correo salen
  // deshabilitados en vez de prometer algo que enviar() va a rechazar.
  if (desvioRoto()) return false;
  const { usuario, clave } = credenciales();
  return Boolean(usuario && clave);
}

/**
 * La otra manera de probar: en vez de no mandar nada, mandarlo TODO a una sola
 * dirección. Con CORREOS_DESVIO=alguien@uniboyaca.edu.co los avisos salen de
 * verdad —se arman, se envían y se ven en una bandeja— pero ninguno llega a la
 * persona a la que iban.
 *
 * Es lo que CORREOS=off no puede dar: apagado se prueba que la app no revienta,
 * desviado se prueba el correo mismo —que el enlace abre, que el código se lee,
 * que la plantilla no se descuadra en el celular—, y eso es justo lo que hay
 * que mirar antes de un evento.
 *
 * CORREOS=off manda sobre esto: el freno de mano se pone encima de todo lo
 * demás, para que apagar sea apagar y no haya que acordarse de dos líneas.
 */
const ES_DIRECCION = /^[^@\s]+@[^@\s]+\.[^@\s]+$/;

function desvio() {
  if (apagados()) return "";
  const v = String(process.env.CORREOS_DESVIO || "").trim();
  return ES_DIRECCION.test(v) ? v : "";
}

// Algo escrito en CORREOS_DESVIO que no es una dirección. Se trata como un
// candado y no como una línea que no se entendió: quien puso eso quería que los
// correos NO salieran a la calle, y un "uniboyaca.edu.co" sin el nombre delante
// —o un "on" de más— no puede acabar en la bandeja de un estudiante por un dedo
// torcido. Se para todo y se dice en la consola.
function desvioRoto() {
  if (apagados()) return false;
  const v = String(process.env.CORREOS_DESVIO || "").trim();
  return Boolean(v) && !ES_DIRECCION.test(v);
}

/**
 * Si el correo le llega a la persona a la que va dirigido.
 *
 * No es lo mismo que activo(), y la diferencia es justo el desvío: con él
 * puesto el aviso SALE —el botón del panel tiene sentido, por eso activo()
 * sigue siendo cierto— pero a la bandeja de quien está probando.
 *
 * Esta es la que tienen que preguntar las páginas públicas antes de decirle a
 * un estudiante "también te llegó por correo": esa frase, dicha mientras el
 * correo está desviado, lo manda a buscar en su bandeja algo que no está.
 */
const aDestinatario = () => activo() && !desvio() && !desvioRoto();

let transporte = null;

// Las quejas de abajo se dicen una sola vez: son de configuración, no de cada
// correo, y repetirlas en cada registro solo ensucia la consola.
let quejaSinClaves = false;
let quejaLocalhost = false;
let quejaDesvioRoto = false;
...
async function enviar({ para, asunto, html, texto, remitente, responder_a }) {
  ...
  if (desvioRoto()) {
    if (!quejaDesvioRoto) {
      quejaDesvioRoto = true;
      console.error(
        `  ! CORREOS_DESVIO="${String(process.env.CORREOS_DESVIO).trim()}" no es una dirección. ` +
          "No sale ningún correo hasta que se arregle o se borre esa línea del .env."
      );
    }
    return false;
  }
  ...
  try {
    await conexion().sendMail({
      ...
      // Desviado, el correo se manda entero a una sola dirección. Lo que iba en
      // el "Para" no se pierde: pasa al asunto y a un renglón arriba del cuerpo,
      // porque veinte correos de prueba en una bandeja son indistinguibles si
      // ninguno dice de quién era.
      to: a || para,
      replyTo: responder_a || CORREO.responder_a || undefined,
      subject: a ? `[iba a ${para}] ${asunto}` : asunto,
      text: a ? `${avisoDesvio(para)}\n\n${texto}` : texto,
      html: a ? bandaDesvio(para) + html : html,
    });
    console.log(a ? `  ✉ ${a} · DESVIADO de ${para} · ${asunto}` : `  ✉ ${para} · ${asunto}`);
    return true;
  } catch (e) {
    console.error(`  ! No se pudo enviar a ${a || para}: ${e.message}`);
    return false;
  }
}

const avisoDesvio = (para) =>
  `*** CORREO DESVIADO — iba dirigido a ${para} y no le llegó. ` +
  "Sale así por CORREOS_DESVIO en el .env. ***";

// La banda del desvío: roja, arriba del todo y con la dirección de verdad. Va
// en el cuerpo y no solo en el asunto, porque estos correos se miran abiertos
// —para eso se desvían: para ver cómo quedaron—. Se monta sobre la misma
// columna de 560 px de la plantilla para que se lea como parte del correo y no
// como algo que se coló.
const bandaDesvio = (para) => `<div style="background:#f4f4f7;padding:24px 12px 0;font-family:${FUENTE}">
  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width:560px;margin:0 auto">
    <tr><td style="background:#a3271c;padding:12px 18px;border-radius:10px;color:#fff;font-size:13px;line-height:1.5">
      <strong>CORREO DESVIADO</strong> · iba dirigido a <strong>${esc(para)}</strong>
      y no le llegó. Sale a esta bandeja por CORREOS_DESVIO en el .env.
    </td></tr>
  </table>
</div>`;
...
function samiAvisoIntencion({ codigo, titulo, estudiantes, sami }, base) {
  ...
  const cuerpo = [
    parrafo(`Hola <strong>${esc(quien.nombre)}</strong>: tus datos quedaron en el registro
      del Semillero de Investigación SAMI.`),
    alerta([
      "Si aún no lo has hecho, lo primero que tienes que hacer es ir a la dirección del programa y notificar en persona que quieres pertenecer al semillero.",
    ]),
    ficha(dondeIr(d)),
