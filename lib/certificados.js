...
function emitirDeMateria(materiaId, periodoId, docenteNombre) {
  ...
  const ranking = rankingDeMateria(materiaId, periodoId);
  ...
  const insertar = db.prepare(`
    INSERT INTO certificados
      (codigo, materia_id, periodo_id, proyecto_id, estudiante, email, proyecto_titulo,
...
function deMateria(materiaId, periodoId) {
  return db
    .prepare(
      `SELECT * FROM certificados WHERE materia_id = ? AND periodo_id = ?
...
// idempotente y se repite cada vez que cambia una nota; avisar, no: por eso el
// aviso es un paso aparte que el docente dispara cuando ya está conforme.
function pendientesDeAviso(materiaId, periodoId) {
  return db
    .prepare(
      `SELECT * FROM certificados
       WHERE materia_id = ? AND periodo_id = ? AND email IS NOT NULL AND avisado_at IS NULL
       ORDER BY estudiante COLLATE NOCASE`
    )
    .all(materiaId, periodoId);
}
 ...
 */
async function avisarPendientes(materiaId, periodoId, base) {
  ...
  for (const cert of pendientesDeAviso(materiaId, periodoId)) {
    const ok = await envios.avisoCertificado(
      { ...cert, etiqueta: etiquetaPuesto(cert.puesto) },
      base
    );
    if (ok) {
      marcar.run(cert.id);
