...
/**
 * La asistencia del día. Son TRES estados y no dos, y esa es toda la idea:
 * "todavía no lo he mirado" no es lo mismo que "no vino". En la puerta del bus
 * se pasa la lista de arriba abajo, y si el sin marcar y el ausente se vieran
 * igual, una lista a medio pasar diría que media clase se quedó en tierra.
 */
const ASISTENCIA = {
...
function conEstado(reg) {
  ...
  return {
    ...
    asistencia: asistenciaDe(reg.asistio),
...
function resumen(salida) {
  const fila = db
    .prepare(
      `SELECT COUNT(*) AS total,
              ...
              -- La asistencia se cuenta SOLO entre los confirmados: quien no
              -- pagó no sube al bus, así que no es un ausente, es otra cosa.
              COALESCE(SUM(pago_transporte = 1 AND pago_poliza = 1 AND asistio = 1), 0) AS asistieron,
              COALESCE(SUM(pago_transporte = 1 AND pago_poliza = 1 AND asistio = 0), 0) AS faltaron
  ...
  return {
    ...
    // Los que todavía no se han mirado. Es la cuenta que dice si la lista ya
    // se pasó o va por la mitad.
    sin_pasar: fila.confirmados - fila.asistieron - fila.faltaron,
