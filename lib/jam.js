...
/**
 * Abre la edición de un semestre. Es el botón de "pasar de semestre": deja la
 * anterior finalizada (con sus equipos y su tema donde están) y crea una
 * nueva, vacía y con las inscripciones abiertas.
 *
 * Los valores por defecto salen de config.JAM, pero se copian a la fila: si
 * un semestre la jam dura 72 horas o el equipo puede ser de cinco, se cambia
 * en esa edición y las viejas siguen contando lo que contaron.
 */
function abrirEdicion({ periodoId, nombre, horas, maxIntegrantes, cupoEquipos }) {
  db.exec("BEGIN");
  try {
    db.prepare("UPDATE jam_ediciones SET estado = 'finalizada', inscripcion_abierta = 0 WHERE estado != 'finalizada'").run();

    const info = db
      .prepare(
        `INSERT INTO jam_ediciones
           (periodo_id, nombre, estado, inscripcion_abierta, horas, max_integrantes, cupo_equipos)
         VALUES (?, ?, 'inscripcion', 1, ?, ?, ?)`
      )
      .run(
        periodoId || null,
        nombre,
        Number(horas) || JAM.horas,
        Number(maxIntegrantes) || JAM.max_integrantes,
        Number(cupoEquipos) || null
      );
...
// Se puede inscribir si la edición lo permite y todavía hay cupo.
...
function entregaAbierta(ed, f = fase(ed)) {
  return Boolean(ed && ed.entregas_abiertas && ed.estado !== "finalizada" && yaArranco(f));
}
